...
// SkillGlobalDir is where `--install --global` writes, relative to the home
// directory — Claude Code's user-level skills, active in every project.
const SkillGlobalDir = ".claude/skills/pilot"

func newSkillCmd(g *globals) *cobra.Command {
	var install, global bool
	...
	cmd := &cobra.Command{
		...
		Long: "Pilot ships instructions for an AI agent operating a fleet: the concepts that\n" +
			...
			"it to " + SkillInstallDir + " for Claude Code to pick up; add `--global` (or\n" +
			"use `-g` alone, which implies --install) to write it to ~/" + SkillGlobalDir + "\n" +
			"instead, where it applies to every project rather than one fleet directory.",
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			return runSkill(g, install || global, global)
		},
	}
	...
	cmd.Flags().BoolVarP(&global, "global", "g", false,
		"install for every project (~/"+SkillGlobalDir+"); implies --install")
	return cmd
}

func runSkill(g *globals, install, global bool) error {
	...
	dir, err := skillInstallDir(g.dir, global)
	if err != nil {
		return err
	}
...
// skillInstallDir resolves where the skill lands: the user's home for a
// global install, otherwise alongside the fleet configuration, since that is
// the directory an agent will be working in.
func skillInstallDir(fleetDir string, global bool) (string, error) {
	if global {
		home, err := os.UserHomeDir()
		if err != nil {
			return "", err
		}
		return filepath.Join(home, SkillGlobalDir), nil
	}
	...
	return filepath.Join(root, SkillInstallDir), nil
}
